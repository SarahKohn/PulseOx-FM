...
# Root directory for your own cohort's tabular data. These CSV/tabular files are
# private cohort data and are NOT distributed with this repository. Point the
# PULSEOX_DATA_DIR environment variable at the directory that holds them; it
# defaults to a `data/` folder at the repository root, e.g.:
#     export PULSEOX_DATA_DIR=/path/to/your/data
DATA_DIR = os.environ.get("PULSEOX_DATA_DIR", os.path.join(REPO_ROOT, "data"))
PATH_FOR_CSV = os.path.join(DATA_DIR, "csv_files")
...
def load_dataset_filenames_dict() -> dict:
    return {'Age_Gender_BMI': os.path.join(PATH_FOR_CSV, 'age_gender_bmi_df.csv'),
            ...
            'baseline_diagnoses_nastya_raw':
                os.path.join(PATH_FOR_CSV, 'baseline_conditions_all.csv'),
