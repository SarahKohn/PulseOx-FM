...
def get_2d_sincos_pos_embed_spectogram(embed_dim, grid_width, grid_height, cls_token=False):
    """
    grid_width: int of the grid width (e.g., time_frames)
    grid_height: int of the grid height (e.g., freq_bins)
    return:
    pos_embed: [grid_width * grid_height, embed_dim] or 
               [1 + grid_width * grid_height, embed_dim] (w/ or w/o cls_token)
    """
    # Create separate grids for height and width
    ...
    grid = np.stack(grid, axis=0)       # Shape: [2, grid_height, grid_width]

    # Reshape to match non-square grid
    grid = grid.reshape([2, 1, grid_height, grid_width])

    # Generate sine-cosine positional embeddings from the grid
    pos_embed = get_2d_sincos_pos_embed_from_grid(embed_dim, grid)

    if cls_token:
        # Add a class token embedding at the beginning
        pos_embed = np.concatenate([np.zeros([1, embed_dim]), pos_embed], axis=0)
...
def get_1d_sincos_pos_embed(emb_dim, grid_size, cls_token=False):
        ...
        Args:
            emb_dim: Dimension of the positional embeddings.
            cls_token: If True, prepends a [CLS] token embedding.
        Returns:
            pos_embed: Positional embeddings of shape [seq_length, embed_dim]
                       or [1 + seq_length, embed_dim] (if cls_token is True).
        """
        # Generate a sequence of positions from 0 to seq_length - 1
        pos = np.arange(grid_size, dtype=np.float32)

        # Compute 1D sin-cos positional embeddings
        pos_embed = get_1d_sincos_pos_embed_from_grid(emb_dim, pos)

        # Add a [CLS] token embedding if requested
        if cls_token:
...
def get_1d_sincos_pos_embed_from_grid(emb_dim, pos):
        """
        Generates the sine-cosine positional encoding for a 1D grid.
        Args:
            emb_dim: output dimension for each position.
            pos: A list of positions to be encoded, shape (M,).
        Returns:
            emb: Positional embeddings of shape (M, embed_dim).
        """
        assert emb_dim % 2 == 0
        omega = np.arange(emb_dim // 2, dtype=np.float32)
        omega /= emb_dim / 2.
