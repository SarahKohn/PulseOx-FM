...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None): #data_loader, print_freq, header
        i = 0
        if not header:
            header = ''
...
def init_distributed_mode(args):
    if args.dist_on_itp:
        ...
        print('RANK and WORLD_SIZE in environ')
        args.rank = int(os.environ["RANK"])
        args.world_size = int(os.environ['WORLD_SIZE'])
        args.gpu = int(os.environ['LOCAL_RANK'])
    elif 'SLURM_PROCID' in os.environ:
        print('SLURM distributed mode')
...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler):
    ...
    if 1:  # always save this checkpoint format
        checkpoint_paths = [output_dir / (f'checkpoint-{wandb.run.id}.pth')]
        ...
        model.save_checkpoint(save_dir=args.output_dir, tag=f"checkpoint-{wandb.run.id}", client_state=client_state)
