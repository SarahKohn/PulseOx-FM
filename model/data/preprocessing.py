...
# Directories for your own cohort's data. None of these files are distributed
# with this repository. Configure via environment variables (defaults are
# repo-relative `data/` folders):
#   PULSEOX_DATA_DIR      processed-data / outputs root
#   PULSEOX_RAW_DATA_DIR  raw sleep recordings root (holds timeseries/ and the mapping files)
DATA_DIR = os.environ.get("PULSEOX_DATA_DIR", os.path.join(REPO_ROOT, "data"))
RAW_DATA_DIR = os.environ.get("PULSEOX_RAW_DATA_DIR", os.path.join(DATA_DIR, "sleep"))
PROJECT_DIR = DATA_DIR
PERSONAL_DIR = DATA_DIR
PYPPG_OUTPUT_DIR = os.path.join(DATA_DIR, "pyPPG_output")
SEED = 30
...
PATH_FOR_DATA = os.path.join(RAW_DATA_DIR, "timeseries")
PATH_FOR_PROCESSED_DATA = os.path.join(PROJECT_DIR, f"preprocessed_data_SR_{RESAMPLE_RATE}Hz")
SIGNALS = [
...
class PreprocessSleepDataset(Dataset):
    def __init__(self, directory,
        ...
        self.uuid_to_id = (
            pd.read_csv(os.path.join(RAW_DATA_DIR, 'participant_id_to_uuid.csv'))
            .assign(**{"Participant ID": lambda df: df["Participant ID"].astype(str)})
            .set_index("UUID")["Participant ID"]
            .to_dict())
        pheno_gold_data = pd.read_parquet(os.path.join(RAW_DATA_DIR, 'sleep_all.parquet')
...
if __name__ == "__main__":
    ...
    #     os.path.join(PYPPG_OUTPUT_DIR, 'pyPPG_biomarkers.parquet'))
